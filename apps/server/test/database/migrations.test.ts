...
describe('migrations & append-only enforcement', () => {
  ...
  beforeAll(async () => {
    ...
    const [project] = await app.query(
      `INSERT INTO projects (owner_id, name, repo_url) VALUES ($1, 'p', 'file:///tmp/r') RETURNING id`,
      [user.id],
    );
    const [agent] = await app.query(
      `INSERT INTO agents (owner_id, name, adapter) VALUES ($1, 'Implementer', 'api-loop') RETURNING id`,
      [user.id],
    );
    const [run] = await app.query(
      `INSERT INTO runs (project_id, agent_id, task_prompt, base_ref) VALUES ($1, $2, 'do it', 'main') RETURNING id`,
      [project.id, agent.id],
    );
    runId = run.id;
  }, 180_000);
  ...
  it('applies cleanly to an empty PG 18 and is idempotent on re-run', async () => {
    const tables: Array<{ table_name: string }> = await admin.query(
  ...
  it('app role can INSERT run_events but UPDATE/DELETE are denied', async () => {
    await app.query(
      `INSERT INTO run_events (run_id, seq, type, payload) VALUES ($1, 1, 'agent.message', '{"text":"hi"}')`,
      [runId],
    );
    await expect(
      app.query(`UPDATE run_events SET payload = '{}' WHERE run_id = $1`, [runId]),
    ).rejects.toThrow(/permission denied/);
    await expect(app.query(`DELETE FROM run_events WHERE run_id = $1`, [runId])).rejects.toThrow(
      /permission denied/,
    );
  });
  ...
  it('even the admin role cannot mutate run_events (trigger)', async () => {
    await expect(
      admin.query(`UPDATE run_events SET payload = '{}' WHERE run_id = $1`, [runId]),
    ).rejects.toThrow(/append-only/);
    await expect(admin.query(`DELETE FROM run_events WHERE run_id = $1`, [runId])).rejects.toThrow(
      /append-only/,
    );
  });
  ...
  it('outbox_events: app may only set dispatched_at; payload is immutable', async () => {
    ...
    await expect(
      app.query(`UPDATE outbox_events SET payload = '{"x":1}' WHERE id = $1`, [row.id]),
    ).rejects.toThrow(/permission denied/);
    // …and by trigger even for admin
    await expect(
      admin.query(`UPDATE outbox_events SET payload = '{"x":1}' WHERE id = $1`, [row.id]),
    ).rejects.toThrow(/immutable/);
    // direct DELETE denied even for admin (must go through prune fn)
    await expect(admin.query(`DELETE FROM outbox_events WHERE id = $1`, [row.id])).rejects.toThrow(
      /append-only/,
    );
  ...
  it('app role cannot read the migrations bookkeeping table', async () => {
    await expect(app.query(`SELECT * FROM typeorm_migrations`)).rejects.toThrow(
      /permission denied/,
    );
  });
});
